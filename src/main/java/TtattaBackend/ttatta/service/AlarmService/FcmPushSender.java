...
public class FcmPushSender {
    // FCM 푸시 알림을 보내는 메서드
    public void sendPushNotification(String token, AlaramType alaramType, String memoryDiaryAlarmDaysAgo, Long diaryId) {
        try {
            // FCM 서버에 요청을 보내는 로직을 구현합니다.
            // 받은 token을 이용하여 fcm를 보내는 메서드
            String title = alaramType.getTitle();
            String body = "";
            Long nearestDiaryId = null;
            if (alaramType.getType().equals("MEMORY_LOCATION")) {
                body = memoryDiaryAlarmDaysAgo;
            }
            body += alaramType.getBody();
